(function () {
  ...
  function isYafName(value) {
    const normalized = String(value || "").trim().toLowerCase().replaceAll("’", "'");
    return /(^|[^a-z])yaf([^a-z]|$)/.test(normalized)
      || normalized.includes("young adult fellowship")
      || normalized.includes("young adults fellowship")
      || normalized.includes("young adults' fellowship");
  }

  function isMensFellowshipName(value) {
    const normalized = String(value || "").trim().toLowerCase().replaceAll("’", "'");
    return /(^|[^a-z])men'?s fellowship([^a-z]|$)/.test(normalized);
  }

  function isWomensFellowshipName(value) {
    const normalized = String(value || "").trim().toLowerCase().replaceAll("’", "'");
    return /(^|[^a-z])women'?s fellowship([^a-z]|$)/.test(normalized);
  }

  function fellowshipLogoFor(group) {
    const names = [state.member?.group_name, group?.name];
    if (names.some(isYafName)) return { src: "assets/pcg-yaf-logo.png", alt: "Presbyterian Church of Ghana Young Adults’ Fellowship (YAF) logo", shapeClass: "member-logo-round" };
    if (names.some(isMensFellowshipName)) return { src: "assets/pcg-mens-fellowship-logo.png", alt: "Presbyterian Church of Ghana Presby Men’s Fellowship logo", shapeClass: "member-logo-square" };
    if (names.some(isWomensFellowshipName)) return { src: "assets/pcg-womens-fellowship-logo.png", alt: "Presbyterian Church of Ghana Presbyterian Women’s Fellowship (P.W.F.) logo", shapeClass: "member-logo-square" };
    return null;
  }
  ...
  function fellowshipMarkup() {
    const group = classification(); const fellowship = state.member.group_name || group.name; const fellowshipLogo = fellowshipLogoFor(group); const fellowshipMark = fellowshipLogo ? fellowshipLogoMarkup(fellowshipLogo, "member-fellowship-logo") : '<span class="member-fellowship-icon"><i data-lucide="users-round"></i></span>'; const relevantEvents = upcomingEvents().filter(event => event.audience_type === "All" || event.audience_group === fellowship || event.audience_group === group.name); const relevantAnnouncements = state.announcements.filter(item => item.audience_type === "All" || item.audience_group === fellowship || item.audience_group === group.name);
  ...
  async function savePassword(event) {
    ...
    if (values.password.length < 12 || values.password.length > 128 || !/[a-z]/.test(values.password) || !/[A-Z]/.test(values.password) || !/\d/.test(values.password) || !/[^A-Za-z0-9]/.test(values.password)) return formError("#memberPasswordError", "Use 12–128 characters with uppercase, lowercase, a number, and a symbol.");
    ...
    try {
      const { error: auditError } = await client.rpc("record_own_security_event", { event_action: "password.changed", event_metadata: {} });
      if (auditError) console.error("Security audit write failed", auditError);
  ...
  async function signOutOtherSessions() {
    ...
    if (error) return toast(error.message, "error");
    try {
      const { error: auditError } = await client.rpc("record_own_security_event", { event_action: "sessions.revoked", event_metadata: {} });
      if (auditError) console.error("Security audit write failed", auditError);
    } catch (auditError) { console.error("Security audit write failed", auditError); }
