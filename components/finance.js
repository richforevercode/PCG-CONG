(function () {
  ...
  function collectionsMarkup() {
    ...
    return `<div class="finance-summary-strip">${metric(forced ? `${forced} total` : "All collections", money.format(total), "Accounted, non-voided records", "hand-coins", "green")}${metric("This month", money.format(monthTotal), currentMonth, "calendar-range", "blue")}${metric("Records", String(records.length), `${memberCount} linked member${memberCount === 1 ? "" : "s"}`, "list-checks", "orange")}${forced === "Adult Offertory" ? metric("District share", money.format(records.reduce((sum,item)=>sum+num(item.district_share),0)), "Historical rule snapshots", "building-2", "red") : metric("Pending", String(state.collections.filter(item => (!forced || item.collection_type === forced) && item.status === "Pending").length), "Awaiting counting", "clock-3", "orange")}</div>${daySummary}<article class="card finance-record-card">${collectionFiltersMarkup()}<div id="financeCollectionTable">${collectionTableMarkup()}</div></article>`;
  }
  ...
  function render() {
    ...
    const details={dashboard:["Overview","Finance & stewardship","Accountable church collections, harvests, expenses, funds, remittance, and reporting."],collections:["All Collections","All Collections","Record and reconcile all church collection types."],tithes:["Tithe","Tithe","Member-linked Tithe history and totals."],vto:[VTO_TYPE,VTO_TYPE,"Track member thanksgiving giving and occasions."],children:["Children Service Offertory","Children Service Offertory","Keep children service collections separate and accountable."],jy:["Junior Youth (JY) Offertory","Junior Youth (JY) Offertory","Junior Youth (JY) Offertory records and reporting."],adult:["Adult Offertory","Adult Offertory","Automatic Local Church and Sebrepor District distribution."],miniHarvest:[MINI_HARVEST_TYPE,MINI_HARVEST_TYPE,"Track Sunday through Saturday Day Born Mini-Harvest records."],mainHarvest:[MAIN_HARVEST_TYPE,MAIN_HARVEST_TYPE,"Create and manage Main Harvest events separately."],remittances:["60/40 Distribution","60/40 Distribution","Track amounts due, remitted, and outstanding."],expenses:["Expenses","Expenses","Pending → Approved → Paid expense workflow."],funds:["Funds / Accounts","Funds / Accounts","Track restricted and unrestricted church funds."],reports:["Financial Reports","Financial Reports","Clear collection, harvest, expense, fund, and remittance reporting."],settings:["Finance Settings","Finance Settings","Configure the Adult Offertory distribution rule."],audit:["Audit Trail","Finance Audit Trail","Immutable accountability history for financial actions."]}[state.section];
