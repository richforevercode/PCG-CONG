...
-- The congregation's district split applies only to Tithe, Adult Offertory,
-- Children Service Offertory, and Junior Youth (JY) Offertory.
with adult_rule as (
  ...
  limit 1
), eligible_type(collection_type) as (
  values ('Children Service Offertory'::text), ('Junior Youth (JY) Offertory'::text)
)
...
begin
  ...
  if new.collection_type in ('Tithe', 'Adult Offertory', 'Children Service Offertory', 'Junior Youth (JY) Offertory') then
    if tg_op = 'INSERT' then
...
-- Apply the newly eligible rule to existing Children Service and Junior
-- Youth (JY) Offertory records. Existing Tithe and Adult snapshots remain unchanged.
...
where collection.collection_type in ('Children Service Offertory', 'Junior Youth (JY) Offertory')
...
-- This explicitly includes Mini-Harvest, Main Harvest, VTO, Sunday Offertory,
-- Thanksgiving, Donation, and Other income.
...
set distribution_rule_id = null,
    ...
    local_share = amount,
    district_share = 0
where collection_type not in ('Tithe', 'Adult Offertory', 'Children Service Offertory', 'Junior Youth (JY) Offertory')
  and (
